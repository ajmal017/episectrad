...
class Fisher(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(fish)):
            if (fish[i] >= ovb and fish[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': fish[i]})
                prevsig = 0
            elif (fish[i-1] > ovb and fish[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': fish[i]})
                prevsig = 1
            elif (fish[i-1] > ovs and fish[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': fish[i]})
                prevsig = 0
            elif (fish[i-1] < ovs and fish[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': fish[i]})
                prevsig = 2
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_fish()
