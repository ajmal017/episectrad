...
class MoneyFlowIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(mfi)):
            if (mfi[i] >= ovb and mfi[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': mfi[i]})
                prevsig = 0
            elif (mfi[i-1] > ovb and mfi[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': mfi[i]})
                prevsig = 2
            elif (mfi[i-1] > ovs and mfi[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': mfi[i]})
                prevsig = 0
            elif (mfi[i-1] < ovs and mfi[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': mfi[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_mfi()
