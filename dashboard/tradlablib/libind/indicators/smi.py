...
class SMIErgodicOscillator(object):
    ...
    def trigger(self):
        ...
        for i in range(1, len(smi)):
            if smi[i-1] > smi_sig[i-1] and smi[i] < smi_sig[i] and smi[i] > 0:
                #buy
                signals_trade_sell.append({'x': i, 'y': smi[i]})
                prevsig = 2
            elif smi[i-1] < smi_sig[i-1] and smi[i] > smi_sig[i] and smi[i] < 0:
                #overbought end, sell start
                signals_trade_buy.append({'x': i, 'y': smi[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_smi()
