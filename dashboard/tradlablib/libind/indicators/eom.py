...
class EaseOfMovement(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(eom)):
            if (eom[i] >= ovb and eom[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': eom[i]})
                prevsig = 0
            elif (eom[i-1] > ovb and eom[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': eom[i]})
                prevsig = 2
            elif (eom[i-1] > ovs and eom[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': eom[i]})
                prevsig = 0
            elif (eom[i-1] < ovs and eom[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': eom[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_eom()
