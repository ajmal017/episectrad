...
class VolumeOscillator(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(vo)):
            if (vo[i] >= ovb and vo[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': vo[i]})
                prevsig = 0
            elif (vo[i-1] > ovb and vo[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': vo[i]})
                prevsig = 2
            elif (vo[i-1] > ovs and vo[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': vo[i]})
                prevsig = 0
            elif (vo[i-1] < ovs and vo[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': vo[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_vo(*params1)
