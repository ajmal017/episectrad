...
class ChandeMomentumOscillator(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(cmo)):
            if (cmo[i] >= ovb and cmo[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': cmo[i]})
                prevsig = 0
            elif (cmo[i-1] > ovb and cmo[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': cmo[i]})
                prevsig = 2
            elif (cmo[i-1] > ovs and cmo[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': cmo[i]})
                prevsig = 0
            elif (cmo[i-1] < ovs and cmo[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': cmo[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_cmo(*params1)
