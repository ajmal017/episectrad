...
class RelativeVigorIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(rvgir)):
            if (rvgir[i] >= ovb and rvgir[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': rvgir[i]})
                prevsig = 0
            elif (rvgir[i-1] > ovb and rvgir[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': rvgir[i]})
                prevsig = 2
            elif (rvgir[i-1] > ovs and rvgir[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': rvgir[i]})
                prevsig = 0
            elif (rvgir[i-1] < ovs and rvgir[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': rvgir[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_rvgi()
