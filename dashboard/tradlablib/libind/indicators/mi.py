...
class MassIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(mi)):
            if (mi[i] >= ovb and mi[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': mi[i]})
                prevsig = 0
            elif (mi[i-1] > ovb and mi[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': mi[i]})
                prevsig = 2
            elif (mi[i-1] > ovs and mi[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': mi[i]})
                prevsig = 0
            elif (mi[i-1] < ovs and mi[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': mi[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_mi(*params1)
