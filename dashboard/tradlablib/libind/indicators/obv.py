...
class OnBalanceVolume(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(obv)):
            if (obv[i] >= ovb and obv[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': obv[i]})
                prevsig = 0
            elif (obv[i-1] > ovb and obv[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': obv[i]})
                prevsig = 2
            elif (obv[i-1] > ovs and obv[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': obv[i]})
                prevsig = 0
            elif (obv[i-1] < ovs and obv[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': obv[i]})
                prevsig = 1
