...
class WilliamsR(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(wpr)):
            if (wpr[i] >= ovb and wpr[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': wpr[i]})
                prevsig = 0
            elif (wpr[i-1] > ovb and wpr[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': wpr[i]})
                prevsig = 2
            elif (wpr[i-1] > ovs and wpr[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': wpr[i]})
                prevsig = 0
            elif (wpr[i-1] < ovs and wpr[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': wpr[i]})
                prevsig = 1
