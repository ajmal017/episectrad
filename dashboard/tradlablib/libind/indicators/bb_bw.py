...
class BollingerBandsBW(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(bb_bw)):
            if (bb_bw[i] >= ovb and bb_bw[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': bb_bw[i]})
                prevsig = 0
            elif (bb_bw[i-1] > ovb and bb_bw[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': bb_bw[i]})
                prevsig = 2
            elif (bb_bw[i-1] > ovs and bb_bw[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': bb_bw[i]})
                prevsig = 0
            elif (bb_bw[i-1] < ovs and bb_bw[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': bb_bw[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        for argset in arglist:
            ...
            maxv = max(pltdt1)
            minv = min(pltdt1)
            ovba = np.arange((maxv+minv)/2, maxv, (maxv-minv)/20, dtype=float)
            ovsa = np.arange((maxv+minv)/2, minv, -(maxv-minv)/20, dtype=float)
