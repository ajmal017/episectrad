...
class BollingerBandsPB(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(bb_pb)):
            if (bb_pb[i] >= ovb and bb_pb[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': bb_pb[i]})
                prevsig = 0
            elif (bb_pb[i-1] > ovb and bb_pb[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': bb_pb[i]})
                prevsig = 2
            elif (bb_pb[i-1] > ovs and bb_pb[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': bb_pb[i]})
                prevsig = 0
            elif (bb_pb[i-1] < ovs and bb_pb[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': bb_pb[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_bb_pb(*params1)
