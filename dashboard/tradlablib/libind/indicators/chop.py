...
class ChoppinessIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(chop)):
            if (chop[i] >= ovb and chop[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': chop[i]})
                prevsig = 0
            elif (chop[i-1] > ovb and chop[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': chop[i]})
                prevsig = 2
            elif (chop[i-1] > ovs and chop[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': chop[i]})
                prevsig = 0
            elif (chop[i-1] < ovs and chop[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': chop[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_chop()
