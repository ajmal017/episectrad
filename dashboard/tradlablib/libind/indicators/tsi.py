...
class TrueStrengthIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(tsi)):
            if (tsi[i] >= ovb and tsi[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': tsi[i]})
                prevsig = 0
            elif (tsi[i-1] > ovb and tsi[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': tsi[i]})
                prevsig = 2
            elif (tsi[i-1] > ovs and tsi[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': tsi[i]})
                prevsig = 0
            elif (tsi[i-1] < ovs and tsi[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': tsi[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_tsi()
