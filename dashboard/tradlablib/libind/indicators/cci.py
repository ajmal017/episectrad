...
class CommodityChannelIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(cci)):
            if (cci[i] >= ovb and cci[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': cci[i]})
                prevsig = 0
            elif (cci[i-1] > ovb and cci[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': cci[i]})
                prevsig = 2
            elif (cci[i-1] > ovs and cci[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': cci[i]})
                prevsig = 0
            elif (cci[i-1] < ovs and cci[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': cci[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_cci()
