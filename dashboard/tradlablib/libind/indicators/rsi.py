...
class RelativeStrengthIndex(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(rsi)):
            if (rsi[i] >= ovb and rsi[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': rsi[i]})
                prevsig = 0
            elif (rsi[i-1] > ovb and rsi[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': rsi[i]})
                prevsig = 2
            elif (rsi[i-1] > ovs and rsi[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': rsi[i]})
                prevsig = 0
            elif (rsi[i-1] < ovs and rsi[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': rsi[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_rsi()
