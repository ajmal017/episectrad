...
class DirectionalMovementIndex(object):
    ...
    def trigger(self):
        ...
        for i in range(1, len(close)):
            if adx_pos[i-1] < adx_neg[i-1] and adx_pos[i] > adx_neg[i]:
                #sell start
                if prevsig != 2:
                    signals_trade_sell.append({'x': i, 'y': (adx_pos[i] + adx_neg[i])/2})
                prevsig = 2
            elif adx_pos[i-1] > adx_neg[i-1] and adx_pos[i] < adx_neg[i]:
                #overbought end, sell start
                if prevsig != 1:
                    signals_trade_buy.append({'x': i, 'y': (adx_pos[i] + adx_neg[i])/2})
                prevsig = 1
