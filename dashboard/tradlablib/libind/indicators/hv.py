...
class HistoricalVolatility(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(hv)):
            if (hv[i] >= ovb and hv[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': hv[i]})
                prevsig = 0
            elif (hv[i-1] > ovb and hv[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': hv[i]})
                prevsig = 1
            elif (hv[i-1] > ovs and hv[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': hv[i]})
                prevsig = 0
            elif (hv[i-1] < ovs and hv[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': hv[i]})
                prevsig = 2
