...
class ChaikinOscillator(object):
    ...
    def trigger(self, ovb, ovs):
        ...
        for i in range(1, len(ch_osc)):
            if (ch_osc[i] >= ovb and ch_osc[i-1] < ovb):
                #overbought start
                signals_ovbs.append({'x': i, 'y': ch_osc[i]})
                prevsig = 0
            elif (ch_osc[i-1] > ovb and ch_osc[i] <= ovb):
                #overbought end, sell start
                signals_ovbe.append({'x': i, 'y': ch_osc[i]})
                prevsig = 2
            elif (ch_osc[i-1] > ovs and ch_osc[i] <= ovs):
                #oversold start
                signals_ovss.append({'x': i, 'y': ch_osc[i]})
                prevsig = 0
            elif (ch_osc[i-1] < ovs and ch_osc[i] >= ovs):
                #oversold end, buy start
                signals_ovse.append({'x': i, 'y': ch_osc[i]})
                prevsig = 1
    ...
    def train(self):
        ...
        if len(params1) > 0:
            params1 = prepare_params_for_co(*params1)
